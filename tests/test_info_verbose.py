...
def test_verbose_shows_ports(project_with_block: ProjectConfig) -> None:
    runner = CliRunner()
    result = runner.invoke(cli, ["info", "--project-dir", str(project_with_block.root), "--verbose"])
...
def test_verbose_json_has_ports(project_with_block: ProjectConfig) -> None:
    runner = CliRunner()
    result = runner.invoke(cli, ["info", "--project-dir", str(project_with_block.root), "--json", "--verbose"])
    assert result.exit_code == 0
