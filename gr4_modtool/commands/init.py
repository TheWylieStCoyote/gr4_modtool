...
def _scan_blocks_root(project_dir: Path, blocks_root: Path, groups: dict, group_blocks: dict, prefix_ref: list) -> None:
...
def scan_project_dir(project_dir: Path) -> dict:
    ...
    if blocks_dir.is_dir():
        _scan_blocks_root(project_dir, blocks_dir, result["groups"], result["group_blocks"], prefix_ref)

    # Layout 2: src/blocks/
    if not result["groups"]:
        src_blocks = project_dir / "src" / "blocks"
        if src_blocks.is_dir():
            _scan_blocks_root(project_dir, src_blocks, result["groups"], result["group_blocks"], prefix_ref)
...
def _format_scan_report(root: Path, detected: dict) -> str:
    ...
    if detected["groups"]:
        ...
        for g, blocks in sorted(detected["group_blocks"].items()):
            ...
            lines.append(f"    {g:<12} ({len(blocks)} block{'s' if len(blocks) != 1 else ''}): {block_list}")
    else:
...
def cmd(project_dir: str | None, yes: bool, dry_run: bool, force: bool) -> None:
    ...
    if config_path.exists() and not force and not dry_run:
        click.echo(f"Error: {config_path} already exists. Use --force to overwrite or 'gr4_modtool info' to inspect.", err=True)
        sys.exit(1)
    ...
    if yes:
        ...
        version = questionary.text("Version:", default=detected["version"]).ask() or detected["version"]
