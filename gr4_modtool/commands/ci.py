...
def cmd(coverage: bool, release: bool, project_dir: str | None) -> None:
    """Write GitHub Actions CI quality workflows."""
    cfg = load_config(Path(project_dir) if project_dir else None)

    if not coverage and not release:
        click.echo("Specify --coverage and/or --release to generate CI workflows.")
        return
