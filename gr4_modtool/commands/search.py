...
@click.option(
    "--topic", default=_DEFAULT_TOPIC, show_default=True, help="GitHub topic tag to filter by."
)
...
@click.option(
    "--token",
    envvar="GR4_MODTOOL_GITHUB_TOKEN",
    default=None,
...
def cmd(
    ...
    try:
        items = search_registry(
            query=query, topic=topic, token=token, limit=limit, no_cache=no_cache
        )
